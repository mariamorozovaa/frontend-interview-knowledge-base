...
Ок, разберём алгоритмы так, как это реально спрашивают на собеседованиях — с **интуицией + примерами + типичными ловушками**.

---

# 🧠 Complexity (сложность алгоритмов)

---

## 1. Что такое Big O и сложность алгоритма?

### 📌 Простое определение:

**Big O — это оценка того, как растёт время выполнения или память алгоритма при увеличении входных данных.**

---

## 💡 Главное:

Мы НЕ измеряем секунды.

Мы измеряем:

- как растёт количество операций
- при росте N
...
| ---------- | ------------------ | ---------------- |
| O(1)       | константная        | доступ к массиву |
| O(log N)   | логарифмическая    | бинарный поиск   |
| O(N)       | линейная           | цикл             |
| O(N log N) | быстрые сортировки | merge sort       |
...
Big O — это **оценка худшего случая (worst case)**

---

# 🧠 2. Что значит сложность по памяти?

---

## 📌 Space Complexity — это:
...
Мы считаем только **дополнительную память**, а не входные данные.
...
👉 используем только переменные → O(1)
...
👉 стек вызовов → O(N) памяти

---

# 🧠 3. Пример: анализ памяти
...
❌ “цикл не влияет на память”

✔ неправильно — влияет, если создаёшь структуры

---

# 🧠 4. Сложность оператора `...[ ]`
...
👉 копируем 3 элемента → O(N)

---

## ❗ Важно:

Spread — это НЕ “O(1) синтаксис”
...
## 1. Какие структуры данных бывают?

Разберём основные + как их объяснять на собесе
...
| -------- | ------------------ |
| enqueue  | O(1)               |
| dequeue  | O(N) в JS array ⚠️ |

---

## 💡 важно:

в реальности используют linked list / deque
...
Object ≠ Map (Map быстрее и безопаснее)
...
| Структура | Операции  | Использование      |
| --------- | --------- | ------------------ |
| Stack     | O(1)      | call stack         |
| Queue     | O(1)/O(N) | задачи, event loop |
| HashMap   | O(1)      | кеш, поиск         |
| Tree      | O(log N)  | DOM, поиск         |
| Graph     | O(V+E)    | связи              |
...
### 1. “Почему Map лучше Object?”

👉 нет коллизий + быстрее

---

### 2. “Почему spread O(N)?”

👉 потому что копирует все элементы

---

### 3. “Почему queue.shift O(N)?”

👉 сдвигает все элементы

---

### 4. “Какая структура лучше для поиска?”
